...
def crawling_articles_from_keyword():
    ...
    try:
        ...
        for i in range(0, len(querys)):
            print (querys[i])
            naver_news_crawler(maxpage, querys[i], sort, start_date, end_date) 
...
def naver_news_crawler(maxpage, query, sort, start_date, end_date):
    ...
    while page <= maxpage:
        url = "https://search.naver.com/search.naver?where=news&query=" + query + "&sort="+sort+"&ds=" + start_date + "&de=" + end_date + "&nso=so%3Ar%2Cp%3Afrom" + s_from + "to" + e_to + "%2Ca%3A&start=" + str(page)
    ...
    for naver_url in article_link:
        naver_original_html = requests.get(naver_url)
